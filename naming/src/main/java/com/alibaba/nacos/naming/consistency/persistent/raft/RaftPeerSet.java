...
public class RaftPeerSet extends MemberChangeListener implements Closeable {
    ...
    public RaftPeer decideLeader(RaftPeer candidate) {
        ...
        SortedBag ips = new TreeBag(); // 1. 简单的数据结构、会记录每个 key 出现的次数
        int maxApproveCount = 0;
        String maxApprovePeer = null;
        for (RaftPeer peer : peers.values()) { // 2. 寻找 出现次数最多的 那个 ip
        ...
        if (maxApproveCount >= majorityCount()) {  // 3. 如果最大的 ip 统一节点数 >= 多数节点就可以成为 LEADER
            RaftPeer peer = peers.get(maxApprovePeer);
            peer.state = RaftPeer.State.LEADER;

            if (!Objects.equals(leader, peer)) { // 4. 如果 leader 确实发生了改变 ...
                leader = peer;
                ApplicationUtils.publishEvent(new LeaderElectFinishedEvent(this, leader, local())); // 5. 基于 Spring Event 实现的 事件系统
